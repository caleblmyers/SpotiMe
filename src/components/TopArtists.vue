    ...
    <div v-if="displayedTopArtist" class="mb-6 w-full max-w-full min-w-0">
      <div class="bg-gray-50 rounded-lg shadow-sm overflow-hidden p-4">
        <div class="flex items-start gap-4">
          <!-- Large Artist Image -->
          <div class="shrink-0 w-1/3 h-w-1/3 bg-gray-200 rounded-lg overflow-hidden">
            <img v-if="displayedTopArtist.images && displayedTopArtist.images[0]"
              :src="displayedTopArtist.images[0].url" :alt="displayedTopArtist.name"
          ...
          <div class="flex-1">
            <div class="flex items-center gap-3 mb-2">
              ...
              <h3 class="text-lg font-bold text-gray-900">{{ displayedTopArtist.name }}</h3>
            </div>

            <div class="space-y-2 mb-4">
              <div>
                <span class="text-sm font-semibold text-gray-700">Followers: </span>
                <span class="text-sm text-gray-600">{{ formatNumber(displayedTopArtist.followers.total) }}</span>
              </div>
              <div>
                <span class="text-sm font-semibold text-gray-700">Popularity: </span>
                <span class="text-sm text-gray-600">{{ displayedTopArtist.popularity }}%</span>
              </div>
              <div>
                <span class="text-sm font-semibold text-gray-700">Rank: </span>
                <span class="text-sm text-gray-600">#{{ getArtistRank(displayedTopArtist.id) }}</span>
              </div>
            </div>

            <div v-if="displayedTopArtist.genres && displayedTopArtist.genres.length > 0">
              <span class="text-sm font-semibold text-gray-700 block mb-2">Genres:</span>
              <div class="flex flex-wrap gap-2">
                <span v-for="genre in displayedTopArtist.genres" :key="genre"
            ...
            <div v-if="displayedTopArtist.external_urls?.spotify" class="mt-4">
              <a :href="displayedTopArtist.external_urls.spotify" target="_blank" rel="noopener noreferrer"
        ...
        <div v-for="artist in rowArtists" :key="artist.id" @click="selectArtistFromRow(artist)"
          ...
          <div class="p-3">
            <div class="flex items-center gap-2 mb-1">
              <div
                class="shrink-0 flex items-center justify-center w-6 h-6 rounded-full bg-green-500 text-white font-bold text-xs">
          ...
          <table class="w-full">
            <thead class="bg-gray-50 sticky top-0 z-10">
              <tr>
                ...
                <th class="px-4 py-3 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">Popularity
                </th>
              </tr>
            </thead>
